...
This document describes the shipped v0.1.7 permission model.
...
| --- | --- |
| `audioCapture` | Requested only when voice dictation needs microphone capture from an extension page. If Hermes audio transcription is unavailable, v0.1.7 can use Browser speech fallback when Chromium exposes Web Speech. |
...
v0.1.7 keeps this host-permission surface unchanged while adding tab-attached panel opening, Chat only/no-context mode, and selected-tab prompt controls. A narrower optional-host-permissions migration is intentionally deferred until it can be shipped without breaking load-unpacked context capture.
...
Hermes Browser Extension v0.1.7 does **not** request:
